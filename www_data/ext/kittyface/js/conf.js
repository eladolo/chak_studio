...
window.kittyface = (function() {
                    ...
                    if (verifyJWT(tmp_content)) {
                        ...
                        twitch.bits.getProducts().then(function(products) {
                            ...
                            if (typeof tmp_content["poweron"] != 'undefined' && tmp_content["poweron"] !== '') {
                                $("#kittyface_power").prop("checked", Number(tmp_content["poweron"]));
                                alerts_tmp.poweron = tmp_content["poweron"];
                            } else {
                                $("#kittyface_power").prop("checked", Number(alerts.poweron));
                                alerts_tmp.poweron = alerts.poweron;
                            }
                            setTimeout(function(){
                                $('#kittyface_power').trigger('change');
                            }, 1000);
                            ...
                            $.each(products, function(index, product) {
                                ...
                                if (typeof tmp_content["tier" + tmp_index + "_img"] != 'undefined' && tmp_content["tier" + tmp_index + "_img"] !== '') {
                                    if (tmp_content["tier" + tmp_index + "_img"].indexOf("img/kitty") > -1) {
    ...
    var _init = function() {
        ...
        $('#kittyface_power').off('change').on('change', function() {
            if ($(this).prop("checked")) {
                $('.kittyface_on, .kittyface_posicion').removeClass('hide');
                twitch.send("broadcast", "application/json", JSON.stringify({ "msg": "kittyfaceOn"}));
            } else {
                $('.kittyface_on, .kittyface_posicion').addClass('hide');
                twitch.send("broadcast", "application/json", JSON.stringify({ "msg": "kittyfaceOff"}));
            }
            $('.btnSave').trigger('click');
        });

        $('.kitty_select').off('change').on('change', function() {
            var tmp_select = $(this);
        ...
        $('.btnTest').addClass('on').addClass('cursor').removeClass('hide').off('click').on('click', function() {
            ...
            //alerts_tmp[tmp_index - 1].video_url = $("#tier" + tmp_index + "_video_url").val();
        ...
        $('.btnSave').addClass('on').addClass('cursor').removeClass('hide').off('click').on('click', function() {
            ...
            tmp_config.poweron = ($("#kittyface_power").prop("checked") ? "1" : "0");
        ...
        twitch.bits.onTransactionComplete(function(TransactionObject) {
            ...
            Materialize.toast("<p>@" + TransactionObject.displayName + " " +  (typeof window.lang[lang_code].tooltip_bits_config_complete != "undefined" ? window.lang[lang_code].tooltip_bits_config_complete : "send a kittyface of")+ " " + TransactionObject.product.cost.amount + " bits</p>", 4000);
        });
    };
