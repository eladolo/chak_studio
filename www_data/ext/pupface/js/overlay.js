window.pupface = (function() {
    ...
    var buildButtons = function() {
        ...
        $.each(alerts, function(index, val) {
            ...
            var tmp_img = (alerts[index].img_url === "" || alerts[index].img_url == "none") ? 'img/pupface100x100.png' : alerts[index].img_url;
        ...
        if(alerts.poweron == "0"){
            ...
            $('.btnShowModal').fadeOut('fast', function() {
                Materialize.toast("pupfaces off", 3000);
            });
        }
    ...
    var _init = function() {
        ...
        $('.btnShowModal').off('click').on('click', function(e) {
            ...
            setTimeout(function() {
                ...
                $('.label_leaderboard_tab_pupfaces').trigger('click');
            }, 300);
        });
        ...
        twitch.bits.onTransactionComplete(function(TransactionObject) {
            ...
            if (user_trigger) {
                ...
                user_trigger = false;
                alertID = undefined;
                Materialize.toast("<p>@" + TransactionObject.displayName + " " +  (typeof window.lang[lang_code] != "undefined" ? window.lang[window.lang_code].tooltip_bits_config_complete : "send a pupface of")+ " " + TransactionObject.product.cost.amount + " bits</p>", 4000);
            }
        ...
        twitch.listen("broadcast", function(target, contentType, object) {
            ...
            if (object.msg == "pupfaceOn") {
                pupface_power = true;
                $('.modal').modal('close');
                $('.btnShowModal').fadeIn('fast', function() {
                    Materialize.toast("pupfaces on", 3000);
                });
            }
        });
